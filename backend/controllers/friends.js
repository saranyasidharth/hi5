  ...
  FollowUser(req, res) {
    const followUser = async () => {
      await User.update(
        {
          _id: req.user._id,
          'contacts.userFollowed': { $ne: req.body.userFollowed }
        },
        {
          $push: {
            contacts: {
      ...
      await User.update(
        {
          ...
          'contacts.follower': { $ne: req.user._id }
        },
        {
          $push: {
            followers: {
            ...
            notifications: {
              ...
              message: `${req.user.username} is now contacts you.`,
              created: new Date(),
              viewProfile: false
            }
          }
    ...
    followUser()
      .then(() => {
        res.status(HttpStatus.OK).json({ message: 'contacts user now' });
      })
      .catch(err => {
        res
